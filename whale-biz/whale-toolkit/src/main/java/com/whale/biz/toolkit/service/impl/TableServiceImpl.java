...
import com.whale.base.constant.GenConstants;
import com.whale.base.exception.BaseException;
import com.whale.base.util.FileUtil;
import com.whale.base.util.StrUtil;
import com.whale.biz.toolkit.config.GenConfig;
import com.whale.biz.toolkit.entity.Column;
import com.whale.biz.toolkit.entity.Table;
import com.whale.biz.toolkit.mapper.TableMapper;
import com.whale.biz.toolkit.service.ColumnService;
import com.whale.biz.toolkit.service.TableService;
import com.whale.biz.toolkit.util.BuilderUtil;
import com.whale.biz.toolkit.util.VelocityInitializer;
import com.whale.biz.toolkit.util.VelocityUtil;
...
import static com.whale.biz.toolkit.util.GenUtil.*;
...
public class TableServiceImpl extends ServiceImpl<TableMapper, Table> implements TableService {
    ...
    @Override
    @Transactional
    public Table getGenTable(String tableName, String tableComment,String alias) {
        Table table = baseMapper.selectOne(new QueryWrapper<Table>().eq("table_name", tableName).orderByDesc("create_time").last("limit 1"));
        if (table != null) {
            ...
            //表列 信息新增
            List<Column> columns = columnService.selectDbColumnsByName(tableName,alias);
        ...
        if (table != null) {
            ...
            for (String template : templates) {
                ...
                if (StrUtil.containsAny(template, "vue", "js")) {
                    localFile = table.getGenPath().replace("\\", "/") + "/whale-ui/" + VelocityUtil.getFileName(template, table).replace("vue/", "src/");
                } else if (StrUtil.contains(template, "sql")) {
                    localFile = table.getGenPath().replace("\\", "/") + "/whale-biz/whale-" + table.getModuleName() + "/sql/" + VelocityUtil.getFileName(template, table);
                    sqlPath = localFile;
                } else {
                    localFile = table.getGenPath().replace("\\", "/") + "/whale-biz/whale-" + table.getModuleName() + "/src/" + VelocityUtil.getFileName(template, table);
                }
