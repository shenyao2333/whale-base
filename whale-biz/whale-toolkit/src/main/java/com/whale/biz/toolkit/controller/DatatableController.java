package com.whale.biz.toolkit.controller;
...
import com.whale.data.enums.DataTypeEnum;
import com.whale.data.util.AliasUtil;
...
public class DatatableController {
        ...
        try {
            if (DataTypeEnum.MYSQL.getType().equals(AliasUtil.getDsType(alias))) {
                sql.append("select table_name tableName, table_comment tableComment, create_time createTime from information_schema.tables where table_schema=?")
                        .append(" and table_name not like 'sys_%' and table_name not like 'qrtz_%' and table_name not like 'act_%' and table_name not like 'dev_%'");
            ...
            System.out.println(sql +  AliasUtil.getDsName(alias));
            list = jdbcTemplate.queryForList(sql.toString(), AliasUtil.getDsName(alias));
            list.forEach(map -> {
    ...
    public R getGenTable(@RequestBody Table table) {
        return R.ok(tableService.getGenTable(table.getTableName(), table.getTableComment(),table.getAlias()));
    }
