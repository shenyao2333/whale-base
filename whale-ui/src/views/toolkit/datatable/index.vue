...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDatasourceList() {
        datasourceList().then(response => {
          ...
          if (this.datasourceOptions.length > 0) {
            ...
            this.alias = this.datasourceOptions[0].alias;
            this.getList();
          }
        });
      },
      ...
      handleAdd() {
        //this.msgWarning("请选择数据库！");
        this.$router.push({path: "/gen/edit", query: {tableName: '', tableComment: '' , alias: undefined}});
      },
      /** 修改按钮操作 */
      handleEdit(row) {
        this.reset();
      ...
      handleConfig(row) {
        ...
        this.$router.push({path: "/gen/edit", query: {tableName: tableName, tableComment: tableComment ,alias:this.alias }});
      },
      /** 提交按钮 */
      submitForm: function () {
